error id: file://<WORKSPACE>/IMDBQueriesSQL.scala:sql.
...
object IMDBQueriesSQL {
  ...
  def ejecutarConsultasSQL(spark: SparkSession): Unit = {
    ...
    mostrarResultado("1", "peliculas genero de terror de 1987 - 2020 ordenada por voto promedio de language ingles de EUA", {
      val query = """
        SELECT original_title, avg_vote
        FROM full_movies
        WHERE genre LIKE '%Horror%'
          AND year BETWEEN 1987 AND 2020
    ...
    mostrarResultado("1", "encontrar pelicula Misery, encontrar su genero y avg_vote", {
      val query = """
        SELECT original_title, genre, avg_vote
        FROM full_movies
        WHERE original_title LIKE '%Misery%'
      """
      spark.sql(query)
    })

    mostrarResultado("1", "peliculas con la palabra the goonies", {
      val query = """
        SELECT original_title, genre, avg_vote
        FROM full_movies
        WHERE original_title LIKE '%The Goonies%'
      """
      spark.sql(query)
    })

    mostrarResultado("1", "peliculas con 3 generos al mismo tiempo: Adventure, Comedy, Family del anio 1985 en adelante con mas puntuacion que 7.8 pais USA", {
      val query = """
        SELECT original_title, genre, avg_vote
        FROM full_movies

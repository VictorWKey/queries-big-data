...
object IMDBQueriesSQL {
  ...
  def ejecutarConsultasSQL(spark: S@@parkSession): Unit = {
    ...
    mostrarResultado("1", "peliculas genre Adventure, Comedy, Family del anio 1985 en adelante con mas puntuacion que 7.8", {
      val query = """
        SELECT original_title, genre, avg_vote
        FROM full_movies
        WHERE genre IN ('Adventure', 'Comedy', 'Family')
          AND year >= 1985
          AND avg_vote > 7.8
      """
      spark.sql(query)
